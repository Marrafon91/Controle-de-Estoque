    ...
    <div className="bg-surface min-h-screen pb-24 px-5 pt-6">
      ...
      <ul className="mb-6 space-y-2">
        {lista.itens.length === 0 ? (
          <li className="rounded-2xl border border-dashed border-slate-200 py-10 text-center">
            <p className="text-sm font-semibold text-slate-500">Lista vazia</p>
            <p className="mt-1 text-xs text-slate-400">
              Toque no + acima para adicionar o primeiro item.
            </p>
            <button
              onClick={() => setModalAberto(true)}
              className="bg-brand-600 mt-4 rounded-xl px-4 py-2 text-xs font-semibold text-white"
            >
              Adicionar item
            </button>
          </li>
